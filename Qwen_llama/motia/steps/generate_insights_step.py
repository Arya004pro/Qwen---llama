...
Uses LLaMA to generate 2-3 plain-English business insights from query results
and anomaly flags before final formatting.
...
config = {
    ...
    "description": "Generates plain-English business insights using LLaMA from result rows and anomalies.",
...
def _call_llama_insights(user_query: str, parsed: dict[str, Any], rows: list[dict[str, Any]], anomalies: dict[str, Any]) -> tuple[list[str], dict]:
...
async def handler(input_data: Any, ctx: FlowContext[Any]) -> None:
    ...
    usage: dict[str, Any] = {}
    try:
        insights, usage = _call_llama_insights(user_query, parsed, results, anomalies)
        if usage:
            log_tokens(ctx, query_id, "GenerateInsights", LLAMA_MODEL, usage)
            await add_tokens_to_state(ctx, query_id, "GenerateInsights", LLAMA_MODEL, usage)
