...
# Colour palette for Chart.js — one per bar (cycles if more than 8)
_PALETTE = [
    "rgba(99,  179, 237, 0.85)",   # blue
    "rgba(104, 211, 145, 0.85)",   # green
    "rgba(246, 173,  85, 0.85)",   # amber
    "rgba(252, 129, 129, 0.85)",   # red
    "rgba(154, 117, 221, 0.85)",   # purple
    "rgba( 79, 209, 197, 0.85)",   # teal
    "rgba(246, 135, 179, 0.85)",   # pink
    "rgba(183, 148, 255, 0.85)",   # lavender
]
...
def _build_bar_chart(labels, values, metric, title, subtitle):
    """Single horizontal bar chart for top/bottom N results."""
    ...
    cfg = {
        ...
        "options": {
            **_CHART_DEFAULTS,
            "indexAxis": "y",   # horizontal bars look better for named entities
            "plugins": {
                **_CHART_DEFAULTS["plugins"],
                ...
                "tooltip": {
                    **_CHART_DEFAULTS["plugins"]["tooltip"],
                    "callbacks": {
                        "label": f"function(c){{return ' {prefix}'+c.raw.toLocaleString('en-IN',{{minimumFractionDigits:2}});}}"
                    },
                },
            },
            "scales": {
...
def _build_comparison_chart(labels, values1, values2, period1, period2, metric, title, subtitle, ranking):
    """Grouped bar chart for comparison queries (two datasets side-by-side)."""
    prefix = "₹" if metric == "revenue" else ""

    if ranking == "aggregate":
        # Two single bars — one per period
        cfg = {
            "type": "bar",
            "data": {
                ...
                "datasets": [{
                    "label": metric.title(),
                    "data":  [values1[0] if values1 else 0,
                              values2[0] if values2 else 0],
            ...
            "options": {
                **_CHART_DEFAULTS,
                ...
                "scales": {
                    ...
                    "y": {
                        **_CHART_DEFAULTS["scales"]["y"],
                        "ticks": {
                            **_CHART_DEFAULTS["scales"]["y"]["ticks"],
                            "callback": f"function(v){{return '{prefix}'+v.toLocaleString('en-IN');}}",
                        },
                    },
                },
        ...
        # Grouped bars: one group per entity, two bars (period1, period2) each
        cfg = {
            "type": "bar",
            "data": {
                "labels": labels,
                ...
                "plugins": {
                    **_CHART_DEFAULTS["plugins"],
                    ...
                    "tooltip": {
                        **_CHART_DEFAULTS["plugins"]["tooltip"],
                        "callbacks": {
                            "label": f"function(c){{return ' '+c.dataset.label+': {prefix}'+c.raw.toLocaleString('en-IN',{{minimumFractionDigits:2}});}}"
                        },
                    },
                },
                "scales": {
...
# ─── text helpers (unchanged from v2) ────────────────────────────────────────
...
async def handler(input_data: Any, ctx: FlowContext[Any]) -> None:
    ...
    if is_comparison:
        ...
        if results_1 or results_2:
            ...
            if ranking == "aggregate":
                ...
                chart_config = _build_comparison_chart(
                    [], v1, v2, period1, period2,
                    metric, chart_title, chart_subtitle, "aggregate"
                )
            else:
                dict1  = {r["name"]: r["value"] for r in results_1}
                dict2  = {r["name"]: r["value"] for r in results_2}
                ...
                chart_config = _build_comparison_chart(
                    names, vals1, vals2, period1, period2,
                    metric, chart_title, chart_subtitle, ranking
                )

        ctx.logger.info("✅ Comparison result formatted", {"queryId": query_id})
    ...
    if not results or (len(results) == 1 and results[0].get("value") is None):
        ...
        # No bar chart for a single number — chart_config stays None
        ...
        chart_config   = _build_bar_chart(
            labels, values, metric, chart_title, chart_subtitle
        )
